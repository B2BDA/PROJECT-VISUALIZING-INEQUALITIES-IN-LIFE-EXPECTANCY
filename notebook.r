...
# Loading data
life_expectancy <- read.csv("datasets/UNdata.csv", na.strings="")
...
subdata<-life_expectancy  %>% filter(Year=='2000-2005')%>%select(Country.or.Area,Subgroup,Value)%>%spread(key=Subgroup, value = Value)
# Taking a look at the first few rows
head(subdata)
